...
- **通道（首发零补丁）**：预置 bundle 插件对（`dsh.client` 行，host 半 + client 半）——host 半监听 `agent/status` 聚合"完成标记"，client 半定时轮询 `host.call('notify:take')` 取回并上报焦点状态；本地回环 1s 间隔开销可忽略；实时性不够再升级 upstream 白名单（需扁平事件，工程量大，后置）
